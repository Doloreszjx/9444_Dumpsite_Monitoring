...
# 加载测试数据集
test_dataset = VOCDataset('VOC2012', transforms=transforms, train=False)
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=True, collate_fn=lambda x: tuple(zip(*x)))

# 加载 Faster R-CNN 模型
model = torchvision.models.detection.fasterrcnn_mobilenet_v3_large_320_fpn(weights=None)
num_classes = 5  # VOC 数据集通常有 20 类目标 + 背景
in_features = model.roi_heads.box_predictor.cls_score.in_features
model.roi_heads.box_predictor = torchvision.models.detection.faster_rcnn.FastRCNNPredictor(in_features, num_classes)

# 加载训练好的模型参数
model.load_state_dict(torch.load('rcnn/save_weights/mobileNetFpn-model-balanced-17.pth', weights_only=True)['model'])
# model.eval()
