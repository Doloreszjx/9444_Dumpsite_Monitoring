...
def create_model(num_classes):
    ...
    backbone = MobileNetV2(weights_path="rcnn/backbone/mobilenet_v2.pth").features
...
def main():
    ...
    if not os.path.exists("rcnn/save_weights"):
        os.makedirs("rcnn/save_weights")
    ...
    train_dataset = VOCDataset(os.path.join(VOC_root, 'train'), 'train.txt', data_transform["train"])
    train_sampler = None
    ...
    val_dataset = VOCDataset(os.path.join(VOC_root, 'test'), 'test.txt', data_transform["train"])
    ...
    torch.save(model.state_dict(), "rcnn/save_weights/pretrain.pth")
    ...
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        if epoch in range(num_epochs+init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "rcnn/save_weights/mobile-model-{}.pth".format(epoch))
